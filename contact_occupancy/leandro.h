...
void read_pdb(char filename[200] , int total_atm,  atom *p){
	...
	sprintf( command, "grep ATOM %s > temporary_pdb_atom ; cut -c 12-21 temporary_pdb_atom > temporary_atm ; cut -c 30-55 temporary_pdb_atom > temporary_coord ; paste temporary_atm temporary_coord | awk '{print $1,$2,$3,$4,$5}' > temporary_read_PDB_coordinates", filename);
	system(command);
...
//		printf("%d %s %s %.3f %.3f %.3f\n", atm, p[atm].atm_name , p[atm].res_name, p[atm].x, p[atm].y, p[atm].z );
	}
